...
TEST_CASE("Compound assignment/multiplication", "[mul_assign]") {
  ...
  SECTION("my_complex") {
    ...
    expr *= 4;
    CHECK_THAT(expr, Prints<ref_t>("{4,0}*C+(1,up)"));
    expr *= 0;
...
TEST_CASE("Multiplication", "[mul]") {
  SECTION("double") {
    ...
    CHECK_THAT((expr_r * 0), Prints<ref_t>("0"));
    CHECK_THAT((0 * expr_r), Prints<ref_t>("0"));
    CHECK_THAT((expr_r * 2), Prints<ref_t>("2*C+(1,up)"));
    CHECK_THAT((2 * expr_r), Prints<ref_t>("2*C+(1,up)"));
  }
  SECTION("complex and double") {
    ...
    CHECK_THAT((expr_c * 0.0), Prints<ref_t>("(0,0)"));
    CHECK_THAT((0.0 * expr_c), Prints<ref_t>("(0,0)"));
    CHECK_THAT((expr_c * 2.0), Prints<ref_t>("(2,0)*C+(1,up)"));
    CHECK_THAT((2.0 * expr_c), Prints<ref_t>("(2,0)*C+(1,up)"));
  }
  SECTION("double and complex") {
    ...
    CHECK_THAT((expr_r * 2.0*I), Prints<ref_t>("(0,2)*C+(1,up)"));
    CHECK_THAT((2.0*I * expr_r), Prints<ref_t>("(0,2)*C+(1,up)"));
  }
  SECTION("my_complex") {
    auto expr = c_dag<my_complex>(1, "up");
    using ref_t = decltype(expr);
    ...
    CHECK_THAT((expr * 2), Prints<ref_t>("{2,0}*C+(1,up)"));
    CHECK_THAT((2 * expr), Prints<ref_t>("{2,0}*C+(1,up)"));
  }
}
